...
## Текущий этап: Диалог №5 — финал Фазы 2, контент школы/сада, миграция WebP

После Диалога №4 (палитра v2.0 + рефакторинг всех 16 секций под фидбэк заказчицы) пришла волна нового контента: расписания сада и школы, меню (как принципы питания), CSV-таблицы от заказчицы. Параллельно выполнена крупная техническая миграция: все фото переведены на WebP (q=85), коллекции `school-life` и `team` расширены, рефакторинг Team-компонента устранил баг «рандомной фотографии». Впереди — Yandex Metrika + 14 целей, финальный QA, кастомный домен.
...
**Block 4 — Yandex Metrika**
- ID счётчика и готовый сниппет получены
- Интеграция: `BaseLayout.astro` перед `</head>` + `<noscript>`-фоллбэк
- 14 целей: клики `tel:`, TG, Макс, VK, Я.Карты; CTA «Записаться» по секциям; открытие FAQ; открытие Lightbox; скролл >50%/>75%; время >60с

**Block 5 — Документация** ← **этот коммит**
Обновление PROGRESS.md (v2.4) и DECISIONS.md под итоги Диалога №5.

**Block 6 — Финальный QA**
- Визуальный обход 16 секций на брейкпоинтах 320 / 768 / 1280
- Исправление QA-01 (SummerCamp object-cover)
- Финальный `pnpm build` + Lighthouse на стейджинге
...
**Тексты от клиента**
- Цена пробного дня в школе (`HowToEnroll`, шаг 03) — оставлено как 2 900 ₽ без уточнения «сад/школа», финализировано Диалогом №5
- Статус лицензии на образовательную деятельность (FAQ #08 удалён, `hasCredential` в SchemaOrg удалён — оба ждут подтверждённой формулировки)
- SMART CLASS = Система развивающего обучения? — обе формулировки оставлены, упоминание в Philosophy и School
- Кружки: «Рондо», «Лыжи», «Вокал» — упомянуты в docx без описаний, статус (действующие/планы) не подтверждён
